...
function getDeckVisibility(
  ...
  return ((state as ExtendedRoomState | null)?.deckVisibility?.[player] ??
...
if (existingMemberInDb) {
  ...
  async function moveCard(cardId: string, toZone: Zone) {
    ...
    movingIds.forEach((movingId) => {
      ...
      nextCardInstances[movingId] = {
        ...
        tapped: false,
        reversed: false,
        stackId: movingIds.length > 1 ? movingCard.stackId : null
      };
    });

    const nextStacks =
      movingIds.length > 1 && card.stackId
        ? {
  ...
  async function setDeckVisibilityStatus(
    ...
    if (currentStatus === status) {
      setMessage(`山札はすでに${deckVisibilityLabel(status)}状態です。`);
      return;
    }
    ...
    setRoomState(nextState);
    clearUndoSnapshot({
      reason: `山札を${deckVisibilityLabel(status)}状態にしました。`
    });
  }
  ...
  function isDeckLocked(player: PlayerSide) {
    return getDeckVisibility(roomState, player) === "locked";
  }
  ...
  async function shuffleDeck(player: PlayerSide) {
    ...
    if (isDeckLocked(player)) {
      setMessage("山札がロック中です。状態変更からロックを解除してください。");
      return;
    }
